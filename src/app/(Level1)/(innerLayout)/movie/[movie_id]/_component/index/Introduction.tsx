...
import { Cast, Crew, MovieID, Trailer } from '../../page';
...
export default function Introduction({FTC, MOVIE, CAST, SCENE}: {FTC:FTCProps, MOVIE: MovieID, CAST: Cast, SCENE: Trailer[]}) {
  ...
  const isHearted = DB.hearted.find((item:{id:number}) => item.id == MOVIE.id);
  const isWatched = DB.watched.find((item:{id:number}) => item.id == MOVIE.id);
  const isRated = DB.rating.find((item: {id: number, rated: number}) => item.id == MOVIE.id);
  ...
  const mapp_dried = driedList.map((item: Crew, index:number) => {
    return(
  ...
  const score = parseFloat(MOVIE.vote_average?.toFixed(1)) * 10;
      ...
      <div style={styles2} className={styls.inner_data_info}>
        ...
        <div style={isSmallDevice ? {background: `rgba(${r},${g},${b}, 1)`} : {}} className={styls.data_info_details}>
          ...
          <div className={styls.data_teb_detz_1}>
            <span className={styls.title_teb}>{MOVIE.title}</span>
            <span className={styls.year_teb}>{formattedYear(MOVIE.release_date)}</span>
          </div>
            ...
            <div className={styls.spread_score}>
              <div className={styls.movieRating}>
                <div style={{background: `rgb(${colorDecode2})`}} className={styls.innerCircle}>
                  <SemiCircle1 $color={`rgb(${colorDecode})`} $score={parseFloat(MOVIE.vote_average.toFixed(1))} />
                  <SemiCircle2 $color={`rgb(${colorDecode})`} $score={parseFloat(MOVIE.vote_average.toFixed(1))} />
              ...
              >
                ...
                <OutsideClickHandler onOutsideClick={() => setRating(false)}>
                  <span className={rating ? `${styls.ratingInput} ${styls.active}` : styls.ratingInput}>
                    <div className={styls.clearRating} onClick={() => handleClearRating(MOVIE.id, currentType)}><FaMinus /></div>
                    <div className={styls.ratingStars}>
                      <StarRatings
                        rating={isRated?.rated || 0}
                        starRatedColor="rgb(255,215,0)"
                        changeRating={(val:number) => handleRating(val, currentType, MOVIE.id)}
                        starDimension='21px'
