...
export default function Backdrops({BACKDROP, type}:{BACKDROP:any, type:string}) {
    ...
    if(type == "movie_serie"){
        ...
        languages = BACKDROP?.map((item:any) => item?.language);
        ...
        mapped_current = BACKDROP.map((item:any, index:number) => {
    ...
    <div className={styles.altTitle_Cont}>
        ...
        <div className={styles.left_nav_bar}>
            <div className={styles.fullcrum_nav}>
                <div style={{background: `${FTC?.DarkVibrant}`}} className={styles.fullcrum_head}>Backdrops</div>
